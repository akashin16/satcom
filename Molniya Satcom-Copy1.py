...
mumbai=ephem.Observer() #Defining ground staion position
obs_latitude = 19.0760 #Mumbai India Latitude
obs_longitude = 72.8777 #Mumbai India Longitude
mumbai.lat = '19.0760'
mumbai.lon = '72.8777'
...
track_date = ephem.Date((2018,12,20))
molniya.compute(mumbai)
print(ephem.Date(track_date))
print("Molniya is at", molniya.az, molniya.alt)
...
print("Molniya is at", molniya.sublong, molniya.sublat)
...
print ("Distance between observer and satellite",molniya.range)
...
for i in range(1,5760): #First exemplary pass
    mumbai.date=track_date
    track_date=mumbai.date + (ephem.minute);
    molniya.compute(mumbai);
    longList.append(molniya.sublong*180.0/np.pi)
    latList.append(molniya.sublat*180.0/np.pi)
    elev_angle.append(molniya.alt*180.0/np.pi)
    azimuth_angle.append(molniya.az*180.0/np.pi)
    dist_sat_obs.append(molniya.range)
    doppler_shift.append((molniya.az*f)/c)
    time.append(i) #time axis
    if molniya.alt*180.0/np.pi > 5 : #computing data during connection time
        visible_latList.append(molniya.sublat*180.0/np.pi)
        visible_longList.append(molniya.sublong*180.0/np.pi)
...
plt.plot(72.877,19.0760,color='blue', linewidth=2, marker='o',transform=ccrs.Geodetic(),)

# Location of satellite

plt.plot(molniya.sublong*180.0/np.pi,molniya.sublat*180.0/np.pi,color='blue', linewidth=4, marker='o',transform=ccrs.Geodetic(),)
